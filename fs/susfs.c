				     ...
				     struct fsnotify_iter_info *iter_info)
{
	...
	struct qstr q_name = { .name = NULL, .len = 0 };
	...
	if (mask & FS_EVENT_ON_CHILD) {
		/* event on child inside a watched directory */
		child_inode = (struct inode *)data;
		dir_inode = inode;   /* parent directory */
		if (file_name) {
			q_name.name = file_name;
			q_name.len = strlen(file_name);
		}
	} else {
		/* event on the watched inode itself */
		child_inode = inode;
		dir_inode = NULL;
	}

	return susfs_handle_sdcard_inode_event(mark, mask, child_inode,
					      dir_inode,
					      file_name ? &q_name : NULL,
					      cookie);
}
